...
class DQN(nn.Module):
    ...
    def __init__(self, h, w, outputs):
        ...
        def conv2d_size_out3(size, kernel_size = 3, stride=2):
            return (size - (kernel_size - 1) - 1) // stride + 1

        convw = conv2d_size_out3(conv2d_size_out2(conv2d_size_out1(w)))
        convh = conv2d_size_out3(conv2d_size_out2(conv2d_size_out1(h)))
        linear_input_size = convw * convh * 64
        self.fullyConnected = nn.Linear(linear_input_size, 512)
    ...
    def forward(self, x):
        ...
        x = F.elu(self.fullyConnected(x.view(x.size(0), -1)))
        return self.head(x.view(x.size(0), -1))

resize = T.Compose([T.ToPILImage(),
                    T.Resize(84, interpolation=Image.CUBIC),#试着在这里直接变成84*84
...
def select_action(state):
    ...
    eps_threshold = EPS_END + (EPS_START - EPS_END) * np.exp(-EPS_DECAY*steps_done )
...
PATH = 'train_history/new_method.pth'
