...
def select_action(state):
    ...
    if sample > eps_threshold:
        with torch.no_grad():
            ...
            m = policy_net(state).max(1)
            return m[0].item(), m[1].view(1, 1)
    else:
        return 0, torch.tensor([[random.randrange(n_actions)]], device=device, dtype=torch.long)
...
for i_episode in range(num_episodes):
    ...
    s = current_screen
    zero = torch.zeros(s.shape).to(device)
    ...
    for t in count():  # count()作用：生成无限序列，从0开始，只有通过显示中断操作使其退出循环，否则一直循环下去
        ...
        q, action = select_action(state)  # 选择动作
        if q != 0:
            q_value += q
            q_count += 1
        '''
        ...
        if reward > 0:
            scores[1] += int(reward)
            print('episode {} : {:d}-{:d}  q_average : {}'.format(i_episode, *scores,
                                                                  q_value / (q_count if q_count != 0 else 1)))
            q_count = 0
            q_value = 0
        elif reward < 0:
            scores[0] -= int(reward)
        ...
        current_screen = get_screen()
        if not done:
            next_state = torch.cat((current_screen, state[0][0].unsqueeze(0).unsqueeze(0),
                                    state[0][1].unsqueeze(0).unsqueeze(0), state[0][2].unsqueeze(0).unsqueeze(0)),
                                   dim=1)
        else:
            next_state = None
        # print(next_state)
